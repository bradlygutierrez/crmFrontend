...
export default function Productos() {
  ...
  const handleEditSubmit = async (data) => {
    try {
      const response = await fetch(`http://localhost:8000/productos/${selectedProducto.id_producto}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      />
      <DataDisplay title="Productos" data={productos} onRowClick={handleRowClick} />
      <FormPopup
        isOpen={isEditing}
